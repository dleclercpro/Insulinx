...
class Insulinx:
    ...
    def getUSBHandle(self):
        ...
        self.usb_handle = usb.core.find(idVendor = self.VENDOR,
    ...
    def sendWriteRequest(self, request):
        ...
        try:
            ...
            sys.exit("There was a problem while communicating with the " +
                "InsuLinx.")
    ...
    def sendReadRequest(self):
        ...
        try:
            # Raw response is saved to object in order to be used later
            self.raw_response = np.array(
                self.usb_handle.read(self.REQUEST_TYPES[1], self.REQUEST_LENGTH,
                    self.REQUEST_TIMEOUT), int)

            self.raw_response_status = [x for x in
                self.raw_response[self.RAW_RESPONSE_STATUS[0] :
                    self.RAW_RESPONSE_STATUS[1]]]
    ...
    def parseRawResponses(self):
        ...
        for i in range(self.n_raw_responses):
            ...
            # Those are erroneous answers?
            elif raw_response[0] == self.RAW_RESPONSE_IGNORE:
                pass
                ...
                # Two last elements are irrelevant
                ...
                # Those do not correspond to blood sugar measurements
                elif response[0] == self.REPORT_SUGAR_NONE:
                    pass
    ...
    def loadResponses(self):
        ...
        for i in range(n_logs):
            self.saved_responses.append(np.array(
                logs[i].replace("\n", "").split(" "), int))
    ...
    def getBloodSugarLevels(self):
        ...
        self.times = np.array([], object)
        self.levels = np.array([], float)

        for i in range(self.n_saved_responses):
            report_entry = self.saved_responses[i]

            time = datetime.datetime(2000 +
            ...
            level = (self.REPORT_SUGAR_CONVERSION *
                report_entry[self.REPORT_SUGAR])
    ...
    def plotBloodSugarLevels(self, title, color):
        ...
        sp.plot(self.times, self.levels, color = color,
            linewidth = 1.25)
...
def main():
    ...
    insulinx.getUSBHandle()

    # Start InsuLinx
    insulinx.start()

    # Initialize communication
    insulinx.initializeCommunication()

    # Get sugar levels
    insulinx.sendWriteRequest(insulinx.REQUEST_ASK_RESULTS)
    insulinx.getRawResponses()
    insulinx.parseRawResponses()
    insulinx.loadResponses()
    insulinx.saveResponses()
    insulinx.getBloodSugarLevels()

    # Generate plot of sugar levels
    insulinx.plotBloodSugarLevels(
        "My Blood Sugar Levels Over the Last Few Months", "red")
